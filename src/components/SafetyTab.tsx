        ...
        <div className="flex flex-col gap-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 mb-6 flex items-center gap-2">
                           <CheckCircle2 size={12} className="text-emerald-500" /> Compliant Documentation
                        </p>
                        <p className="text-5xl font-bold text-slate-900 tracking-tighter leading-none">{validDocs.length.toString().padStart(2, '0')}</p>
                        <p className="text-[9px] font-bold text-emerald-600 uppercase tracking-wide mt-6 bg-emerald-50 inline-block px-3 py-1 rounded-lg">Audit Registry OK</p>
                    </div>
                </div>
                ...
                <div className="bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className={`text-5xl font-bold tracking-tighter leading-none ${expiredDocs.length > 0 ? 'text-amber-500' : 'text-slate-200'}`}>
                            {expiredDocs.length.toString().padStart(2, '0')}
                        </p>
                        <p className={`text-[9px] font-bold uppercase tracking-wide mt-6 inline-block px-3 py-1 rounded-lg ${expiredDocs.length > 0 ? 'bg-amber-50 text-amber-600 animate-pulse' : 'bg-slate-50 text-slate-400'}`}>
                           Revision Required
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-slate-900 p-10 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-slate-500 mb-6 flex items-center gap-2">
                           <ShieldCheck size={12} className="text-blue-400" /> Global Safety Rating
                        </p>
                        <p className="text-3xl font-bold uppercase tracking-tighter leading-none italic">
                        ...
                        <div className="flex items-center gap-3 mt-8">
                           ...
                           <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Active Site Audit</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-xl shadow-slate-900/5 flex flex-col xl:flex-row items-center gap-10 relative overflow-hidden group">
                ...
                <div className="flex-1 text-center xl:text-left z-10">
                    <h3 className="text-3xl font-bold text-slate-900 tracking-tighter uppercase mb-3 leading-none">AI Safety Copilot <span className="text-blue-600 italic">POS Architect</span></h3>
                ...
                <button 
                  ...
                  className="w-full xl:w-auto bg-blue-600 hover:bg-blue-700 text-white px-10 py-5 rounded-2xl font-bold text-xs uppercase tracking-wide transition-all shadow-xl shadow-blue-900/20 active:scale-95 flex items-center justify-center gap-3 z-10"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div className="lg:col-span-2 flex flex-col gap-6">
                    <div className="flex justify-between items-center px-4">
                        <h4 className="font-bold text-slate-400 uppercase text-[10px] tracking-wide">Safety Asset Ledger</h4>
                    </div>
                    ...
                    {plans.length === 0 ? (
                        <div className="bg-slate-50/30 rounded-[2.5rem] border-2 border-dashed border-slate-100 py-24 text-center">
                            ...
                            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-wide">No safety artifacts deployed</p>
                        </div>
                    ) : (
                            ...
                            {plans.map(plan => (
                                <div key={plan.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex justify-between items-center group hover:border-blue-600/30 transition-all">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <p className="font-bold text-slate-900 text-lg uppercase tracking-tighter leading-none">{plan.title}</p>
                                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide mt-2 flex items-center gap-2">
                ...
                <div className="flex flex-col gap-6">
                    <h4 className="font-bold text-slate-400 uppercase text-[10px] tracking-wide px-4">Compliance Watch</h4>
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                        <div className="bg-slate-50/50 px-8 py-6 border-b border-slate-100 flex items-center gap-3">
                            <ShieldCheck size={20} className="text-blue-600" />
                            <span className="font-bold text-[10px] uppercase tracking-wide text-slate-900">Resource Compliance</span>
                        </div>
                                ...
                                return (
                                    <div key={doc.id} className="flex gap-5 items-start group/doc transition-all">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-xs font-bold text-slate-900 uppercase tracking-tight flex items-center gap-2 truncate group-hover/doc:text-blue-600 transition-colors">
                                            ...
                                            <p className={`text-[9px] font-bold uppercase mt-2 tracking-wide ${isExpired ? 'text-amber-600' : 'text-emerald-600'}`}>
            ...
            <SlideOver 
                ...
                title={<div className="flex items-center gap-3 text-slate-900 font-bold uppercase tracking-tighter text-2xl">⚡ <span className="italic text-blue-600">AI Safety Copilot</span></div>}
            >
                <div className="pb-20">
