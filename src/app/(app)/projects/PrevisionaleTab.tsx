  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 uppercase tracking-tighter leading-none">Financial Projections</h2>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mt-2">Cash Flow Planning & Margin Control</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-4">Total Projected Inflow</p>
            <p className="text-3xl font-bold text-emerald-600 tracking-tighter">€ {entrate.toLocaleString('it-IT')}</p>
            <div className="flex items-center gap-3 text-[10px] text-emerald-700 font-bold uppercase tracking-wide mt-4">
        ...
        <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-8 relative overflow-hidden group">
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-4">Total Projected Outflow</p>
            <p className="text-3xl font-bold text-red-500 tracking-tighter">€ {uscite.toLocaleString('it-IT')}</p>
            <div className="flex items-center gap-3 text-[10px] text-red-700 font-bold uppercase tracking-wide mt-4">
        ...
        <div className={`rounded-[2.5rem] border p-8 relative overflow-hidden group shadow-2xl ${margine >= 0 ? 'bg-slate-900 border-slate-800 text-white' : 'bg-red-600 border-red-700 text-white'}`}>
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-bold opacity-40 uppercase tracking-wide mb-4">Net Operational Margin</p>
            <p className="text-3xl font-bold tracking-tighter">{margine >= 0 ? '+' : ''}€ {margine.toLocaleString('it-IT')}</p>
            <div className="flex items-center gap-3 text-[10px] opacity-60 font-bold uppercase tracking-wide mt-4">
      ...
      {sorted.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 p-24 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">No Projections Found</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="text-left px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Entry Date</th>
                <th className="text-left px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Classification</th>
                <th className="text-right px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Amount</th>
              ...
              {sorted.map(p => (
                <tr key={p.id} className="hover:bg-slate-50/50 group transition-all">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-3 font-bold text-slate-900 uppercase tracking-tight text-sm">
                  ...
                  <td className="px-8 py-6">
                    <div className="flex flex-col gap-1.5">
                       <span className={`w-fit px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wide border ${
                  ...
                  <td className={`px-8 py-6 text-right font-bold text-lg tracking-tighter ${p.tipo === 'ENTRATA' ? 'text-emerald-600' : 'text-red-500'}`}>
      ...
      <SlideOver 
        ...
        title={<div className="flex items-center gap-3 text-slate-900 font-bold uppercase tracking-tighter text-2xl">📈 <span className="italic text-blue-600">Financial Entry</span></div>}
      >
        <form onSubmit={handleSubmit} className="space-y-10 pb-20">
          <div className="bg-slate-900 p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden group">
             ...
             <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-2 relative z-10 leading-none">Ledger Classification</p>
             <h4 className="text-lg font-bold uppercase tracking-tighter relative z-10">Cash Flow Direction</h4>
             
             <div className="flex p-1.5 bg-white/5 rounded-2xl border border-white/10 mt-6 relative z-10">
              {['ENTRATA', 'USCITA'].map(t => (
                <button key={t} type="button" onClick={() => setTipo(t)}
                  className={`flex-1 py-4 rounded-xl text-[10px] font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-3 ${
                    tipo === t
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Expected Date *</label>
              <input 
                type="date" 
                name="data" 
                required 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Valuation (€) *</label>
              <div className="relative group">
                 <input 
                  ...
                  required 
                  placeholder="0.00"
                  className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 text-xl font-bold text-slate-900 outline-none focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 transition-all shadow-sm text-right pr-12" 
                />
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Transaction Category</label>
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Entry Description / Memo</label>
            <input 
              type="text" 
              name="descrizione" 
          ...
          <div className="pt-10 space-y-4">
            <button 
              type="submit" 
              disabled={isPending}
              className={`w-full text-white py-6 rounded-[2.5rem] text-sm font-bold uppercase tracking-wide shadow-2xl transition-all transform active:scale-95 flex items-center justify-center gap-4 ${
            ...
            <button 
              ...
              className="w-full py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wide hover:text-slate-600 transition-colors"
            >
              Cancel Transaction
            </button>
          </div>
