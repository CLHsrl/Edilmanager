  ...
  return (
    <div className="flex flex-col gap-10">
        ...
        <div className="lg:col-span-7 bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
           ...
           <div className="flex justify-between items-start mb-8 relative z-10">
             <div>
               ...
               <h2 className="text-3xl font-bold text-slate-900 uppercase tracking-tighter leading-none">Emission Control</h2>
             </div>
             <button 
                onClick={() => setIsSlideOpen(true)} 
                className="bg-slate-900 hover:bg-blue-600 text-white px-8 py-4 rounded-2xl flex items-center gap-3 text-[10px] font-bold uppercase tracking-wide shadow-2xl transition-all transform active:scale-95"
             >
           ...
           <div className="flex items-center gap-6 pt-4 relative z-10">
             ...
             <div>
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">Capitale Accertato</p>
               <p className="text-4xl font-bold text-slate-900 tracking-tighter">€ {totaleAccertato.toLocaleString('it-IT')}</p>
             </div>
           </div>
        </div>
          ...
          <div className="lg:col-span-5 bg-slate-900 rounded-[2.5rem] shadow-2xl p-10 text-white flex flex-col justify-between relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-end mb-4">
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Avanzamento Certificato</span>
                <span className="text-4xl font-bold text-blue-400 tracking-tighter leading-none">{avanzamento.toFixed(1)}%</span>
              </div>
            ...
            <div className="flex justify-between items-center relative z-10 pt-4 border-t border-white/5">
              <div>
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Rimanenza</p>
                <p className="text-sm font-bold">€ {(budgetTotale - totaleAccertato).toLocaleString('it-IT')}</p>
              </div>
              <div className="text-right">
                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Budget Totale</p>
      ...
      <div className="grid grid-cols-1 gap-6">
        {sals.length === 0 ? (
          <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 p-24 text-center">
            ...
            <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">Nessun Dato Rilevato</h3>
            ...
            <div key={sal.id} className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm p-10 hover:shadow-xl hover:border-blue-600/20 transition-all group relative overflow-hidden">
              ...
              <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-8 relative z-10">
                <div className="flex items-center gap-8">
                  <div className="bg-slate-900 text-white w-16 h-16 rounded-3xl flex items-center justify-center font-bold text-xl shadow-xl shadow-slate-900/10 group-hover:scale-110 transition-transform">
                    #{sal.numero}
                  </div>
                  <div>
                    <div className="flex items-center gap-4 mb-2">
                       <h4 className="font-bold text-slate-900 uppercase tracking-tighter text-lg">Milestone Certificata</h4>
                       <span className={`text-[9px] font-bold px-3 py-1 rounded-full border uppercase tracking-wide ${
                ...
                <div className="flex flex-col sm:flex-row items-center gap-8 w-full xl:w-auto">
                  <div className="text-right w-full sm:w-auto">
                    <p className="text-3xl font-bold text-slate-900 tracking-tighter leading-none">€ {sal.importo.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</p>
                    <p className="text-[10px] font-bold text-emerald-500 uppercase tracking-wide mt-1 italic">Valore Accertato</p>
                  </div>
                  ...
                  <div className="flex items-center gap-3 w-full sm:w-auto">
                    {sal.stato !== 'FATTURATO' ? (
                      <button 
                        ...
                        disabled={isPending}
                        className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-6 py-3.5 rounded-2xl text-[10px] font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 shadow-xl shadow-blue-900/10 active:scale-95"
                      >
                        <Receipt size={14} /> Fattura 1-Click
                      </button>
                    ) : (
                       <div className="flex-1 sm:flex-none bg-slate-50 text-slate-400 px-6 py-3.5 rounded-2xl text-[10px] font-bold uppercase tracking-wide border border-slate-100 flex items-center gap-2">
                  ...
                  {sal.voci.map(v => (
                    <div key={v.id} className="bg-slate-50/50 p-6 rounded-[1.5rem] border border-slate-50 hover:border-blue-600/10 transition-all">
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">{v.descrizione}</span>
                        <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md">{v.percentuale}%</span>
                      </div>
                      ...
                      <p className="text-lg font-bold text-slate-900 tracking-tighter">€ {v.importoVoce.toLocaleString('it-IT')}</p>
                    </div>
                  ))}
                </div>
              )}
              ...
              {sal.note && (
                <div className="mt-8 p-6 bg-slate-50 rounded-3xl border border-slate-100 relative">
                  ...
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wide mb-2 ml-7">Rilievi & Verbali</p>
      ...
      <SlideOver 
        ...
        title={<div className="flex items-center gap-3 text-slate-900 font-bold uppercase tracking-tighter text-2xl">💰 <span className="italic text-blue-600">Nuovo SAL</span></div>}
      >
        <form onSubmit={handleSubmit} className="space-y-10 pb-20">
          <div className="bg-slate-900 p-8 rounded-[2.5rem] text-slate-400 text-[11px] font-medium leading-relaxed shadow-2xl relative overflow-hidden">
            ...
            <p className="relative z-10">
              La certificazione di un <strong className="text-white uppercase font-bold">SAL</strong> costituisce il titolo per la riscossione dei compensi maturati. Assicurarsi che i dati siano conformi al giornale dei lavori.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Data di Rilevazione *</label>
              <input 
                type="date" 
                name="data" 
                required 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Importo Complessivo (€) *</label>
              <input 
                type="number" 
                name="importo" 
                step="0.01" 
                required 
                placeholder="0.00"
                className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-6 py-4 text-xl font-bold text-slate-900 outline-none focus:ring-4 focus:ring-blue-600/5 focus:border-blue-600 transition-all shadow-sm" 
              />
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Note & Verbali</label>
            <textarea 
              name="note" 
              rows={3} 
          ...
          <div className="bg-white border border-slate-100 rounded-[2.5rem] overflow-hidden shadow-sm">
            <div className="bg-slate-50 px-10 py-6 border-b border-slate-100 flex justify-between items-center">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Scomposizione Analitica</span>
              <button 
                type="button" 
                onClick={addVoce} 
            ...
            <div className="p-8 space-y-6">
              {voci.map((v, i) => (
                <div key={i} className="flex flex-col md:flex-row gap-6 items-center bg-slate-50/50 p-6 rounded-3xl border border-slate-50 relative group transition-all hover:border-blue-100">
                  <div className="flex-1 w-full">
                     <label className="text-[9px] font-bold text-slate-300 uppercase tracking-wide mb-1 block ml-1">Voce / Categoria</label>
                     <input 
                        type="text" 
                  ...
                  <div className="flex gap-6 w-full md:w-auto">
                    <div className="w-24 relative">
                       <label className="text-[9px] font-bold text-slate-300 uppercase tracking-wide mb-1 block ml-1">%</label>
                       <input 
                          type="number" 
                          placeholder="0" 
                          value={v.percentuale} 
                          ...
                          className="w-full bg-white border border-slate-100 rounded-xl pl-3 pr-8 py-2.5 text-xs font-bold text-blue-600 outline-none focus:border-blue-600 transition-all text-right shadow-sm" 
                       />
                       <span className="absolute right-3 top-10 text-[10px] font-bold text-slate-300">%</span>
                    </div>
                    <div className="w-32">
                       <label className="text-[9px] font-bold text-slate-300 uppercase tracking-wide mb-1 block ml-1">Importo</label>
                       <input 
                          type="number" 
                          placeholder="0.00" 
                          value={v.importoVoce} 
                          ...
                          className="w-full bg-white border border-slate-100 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-900 outline-none focus:border-blue-600 transition-all text-right shadow-sm" 
                       />
                    </div>
                  </div>
                  {voci.length > 1 && (
              ...
              {voci.length === 0 && (
                <p className="text-center py-10 text-[10px] font-bold text-slate-200 uppercase tracking-wide italic">Compilazione analitica opzionale</p>
              )}
            </div>
          </div>
