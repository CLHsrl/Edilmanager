...
export default function BudgetAnalysisTab({ projectId }: { projectId: string }) {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      <p className="text-xs font-bold text-slate-400 uppercase tracking-wide animate-pulse">AI Guardian sta analizzando i dati...</p>
    </div>
  );
    ...
    <div className="flex flex-col gap-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-6 relative z-10">Operational Margin</p>
          <div className="flex items-end justify-between relative z-10">
            <div>
              <p className={`text-3xl font-bold tracking-tighter ${summary.currentMargin >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-6 relative z-10">Material Logistics</p>
          <p className="text-3xl font-bold tracking-tighter text-slate-900 relative z-10">{summary.actualMaterials.toLocaleString('it-IT')} €</p>
          <div className="relative z-10 mt-6">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[9px] font-bold text-slate-400 uppercase">Budget Utilization</span>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-6 relative z-10">Labor Deployment</p>
          <p className="text-3xl font-bold tracking-tighter text-slate-900 relative z-10">{summary.actualLabor.toLocaleString('it-IT')} €</p>
          <div className="flex items-center gap-3 mt-6 bg-amber-50 px-4 py-2 rounded-xl w-fit relative z-10">
             <Zap size={14} className="text-amber-500 animate-pulse" />
             <p className="text-[9px] font-bold text-amber-700 uppercase tracking-wide">Real-time Site Sync</p>
          </div>
        </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm relative overflow-hidden group">
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-6 relative z-10">Certified Value (SAL)</p>
          <p className="text-3xl font-bold tracking-tighter text-slate-900 relative z-10">{summary.certifiedRevenue.toLocaleString('it-IT')} €</p>
          <div className="flex items-center gap-3 mt-6 bg-blue-50 px-4 py-2 rounded-xl w-fit relative z-10">
             <CheckCircle size={14} className="text-blue-600" />
             <p className="text-[9px] font-bold text-blue-700 uppercase tracking-wide">Accounting Aligned</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-10">
        ...
        <div className="xl:col-span-8 space-y-8">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-2xl font-bold text-slate-900 uppercase tracking-tighter flex items-center gap-4 leading-none">
                <ShieldAlert size={28} className="text-blue-600" /> Anomaly Intelligence
              </h3>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mt-2">Continuous Site Monitoring Feed</p>
            </div>
            <div className="px-5 py-2 bg-slate-900 text-white rounded-full text-[9px] font-bold uppercase tracking-wide flex items-center gap-2 border border-slate-800 shadow-xl">
          ...
          {anomalies.length === 0 ? (
            <div className="bg-white border border-slate-100 p-20 rounded-[2.5rem] flex flex-col items-center text-center shadow-sm">
               ...
               <h4 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">Perfect Project Health</h4>
                  ...
                  <div className="flex flex-col md:flex-row items-start justify-between gap-8">
                    <div className="flex-1 space-y-4">
                      <div className="flex items-center gap-3">
                        <span className="px-3 py-1 rounded-lg bg-white/60 text-[10px] font-bold uppercase tracking-wide border border-current">
                          {anno.type}
                        </span>
                        <div className="h-1 w-1 bg-current opacity-30 rounded-full"></div>
                        <span className="text-[10px] font-bold uppercase tracking-wide opacity-80 flex items-center gap-2">
                           Priority: <span className="underline decoration-2">{anno.severity}</span>
                        </span>
                      </div>
                      <p className="text-xl font-bold leading-tight tracking-tight uppercase">{anno.description}</p>
                      <div className="bg-white/50 p-6 rounded-[1.5rem] border border-current/10 relative group/action">
                        <p className="text-[10px] font-bold uppercase tracking-wide opacity-40 mb-2">AI Remediation Plan</p>
        ...
        <div className="xl:col-span-4 space-y-8">
           <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wide flex items-center gap-3 ml-2">
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white space-y-8 shadow-2xl relative overflow-hidden group">
              ...
              <div className="space-y-3 relative z-10">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Estimated At Completion (EAC)</p>
                ...
                <div className="flex items-center gap-2 text-[9px] font-bold text-blue-400 uppercase tracking-wide pt-2">
              ...
              <div className="pt-8 border-t border-white/10 space-y-4 relative z-10">
                <button className="w-full bg-white/5 hover:bg-white/10 text-white px-6 py-5 rounded-2xl flex items-center justify-between group transition-all border border-white/5">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wide">Recalculate Budget</span>
                  </div>
                ...
                <button className="w-full bg-blue-600 hover:bg-blue-700 text-white px-6 py-5 rounded-2xl flex items-center justify-between group transition-all shadow-xl shadow-blue-600/20">
                  <div className="flex items-center gap-4">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-wide text-white">Generate Auditor Report</span>
                  </div>
           ...
           <div className="bg-white border border-slate-100 rounded-[2.5rem] p-10 shadow-sm group">
              <div className="flex items-center gap-5 mb-10">
                ...
                <div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">AI Intelligence</p>
                  <p className="text-xl font-bold text-slate-900 uppercase tracking-tight">Health Score</p>
                </div>
              </div>
              
              <div className="space-y-6">
                <div className="flex justify-between items-end">
                  <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Financial Integrity Index</p>
                  <p className="text-2xl font-bold text-blue-600 tracking-tighter">82<span className="text-xs text-slate-300">/100</span></p>
                </div>
