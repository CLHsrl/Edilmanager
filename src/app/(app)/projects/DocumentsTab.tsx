  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 uppercase tracking-tighter leading-none">Document Archive</h2>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wide mt-2">Design Versioning & Compliance Registry</p>
        </div>
        <button 
          onClick={() => setIsSlideOpen(true)}
          className="w-full md:w-auto bg-slate-900 hover:bg-blue-600 text-white px-8 py-4 rounded-2xl flex items-center justify-center gap-3 text-xs font-bold uppercase tracking-wide transition-all shadow-xl shadow-slate-900/10 transform active:scale-95"
        >
        ...
        {categories.map(cat => {
          ...
          return (
            <div key={cat.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm hover:border-blue-200 transition-all cursor-pointer group">
              ...
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-1">{cat.label}</p>
              <p className="text-3xl font-bold text-slate-900 tracking-tighter">{count.toString().padStart(2, '0')}</p>
            </div>
          );
        })}
      </div>
      ...
      {documents.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100 p-24 text-center">
          ...
          <h3 className="text-xl font-bold text-slate-900 uppercase tracking-tighter">No Documentation Available</h3>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100">
                <th className="text-left px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Resource Name</th>
                <th className="text-left px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Classification</th>
                <th className="text-left px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Latest Revision</th>
                <th className="text-left px-8 py-6 text-[10px] font-bold text-slate-400 uppercase tracking-wide">Approval Status</th>
                <th className="px-8 py-6 text-right text-[10px] font-bold text-slate-400 uppercase tracking-wide">Management</th>
              </tr>
            </thead>
                  ...
                  <tr key={doc.id} className="hover:bg-slate-50/50 transition-all group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-bold text-slate-900 uppercase tracking-tight text-sm">{doc.name}</p>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide mt-1 flex items-center gap-2">
                    ...
                    <td className="px-8 py-6">
                      <span className={`px-3 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wide border ${cat?.color} border-current`}>
                        {cat?.label}
                      </span>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-3">
                        <span className="bg-slate-900 text-white text-[10px] px-2 py-0.5 rounded-lg font-bold tracking-wide">V{latestVersion?.versionNumber}</span>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-2.5">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wide text-slate-700">{doc.status}</span>
                      </div>
                    </td>
      ...
      <SlideOver isOpen={isSlideOpen} onClose={() => setIsSlideOpen(false)} title={<div className="flex items-center gap-3 text-slate-900 font-bold uppercase tracking-tighter text-2xl">📁 <span className="italic text-blue-600">Asset Ingestion</span></div>}>
        <form onSubmit={handleCreateDoc} className="space-y-10 pb-20">
          <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 space-y-6">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Document Identifier *</label>
              <input 
                type="text" 
                name="name" 
                required 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Vault Link / Cloud URI *</label>
              <input 
                type="text" 
                name="fileUrl" 
                required 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide ml-1">Initial Release Notes</label>
              <textarea 
                name="notes" 
                rows={3} 
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={isPending}
              className="w-full bg-slate-900 hover:bg-blue-600 disabled:opacity-50 text-white py-6 rounded-[2rem] font-bold text-xs uppercase tracking-wide transition-all shadow-2xl shadow-slate-900/20 flex items-center justify-center gap-3"
            >
      ...
      <SlideOver isOpen={isVersionSlideOpen} onClose={() => setIsVersionSlideOpen(false)} title={<div className="flex items-center gap-3 text-slate-900 font-bold uppercase tracking-tighter text-2xl">⏳ <span className="italic text-blue-600">Revision Ledger</span></div>}>
        <div className="space-y-12 pb-20">
          <form onSubmit={handleAddVersion} className="bg-blue-50 p-8 rounded-[2.5rem] border border-blue-100 space-y-8 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              <div className="space-y-1">
                <h3 className="text-xs font-bold text-blue-700 uppercase tracking-wide">Deploy Revision</h3>
                <p className="text-[10px] font-medium text-blue-400 uppercase tracking-wide">Bump to version v{(selectedDoc?.versions[0]?.versionNumber || 0) + 1}</p>
              </div>
              
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-blue-400 uppercase tracking-wide ml-1">New Asset URI</label>
                <input 
                  type="text" 
                  name="fileUrl" 
                  required 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold text-blue-400 uppercase tracking-wide ml-1">Revision Changelog</label>
                <textarea 
                  name="notes" 
                  rows={2} 
                  required 
              ...
              <button 
                type="submit" 
                disabled={isPending}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-5 rounded-[1.5rem] font-bold text-xs uppercase tracking-wide transition-all shadow-xl shadow-blue-900/10"
              >
          ...
          <div className="space-y-6 px-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wide flex items-center gap-3">
              ...
              {selectedDoc?.versions.map(v => (
                <div key={v.id} className="bg-white border border-slate-100 rounded-3xl p-6 flex items-center justify-between group/v transition-all hover:border-slate-200">
                  <div className="flex items-center gap-5">
                    <div className="w-14 h-14 bg-slate-900 text-white rounded-2xl flex items-center justify-center font-bold text-sm shadow-xl group-hover/v:scale-110 transition-transform">
                      V{v.versionNumber}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-slate-900 uppercase tracking-tight">{v.notes || 'System Update'}</p>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wide mt-1 flex items-center gap-2">
