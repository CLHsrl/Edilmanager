  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8 pb-10">
      ...
      <div className="space-y-6">
        <div>
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">
          ...
          <input type="text" name="soggetto" required defaultValue={mockData?.soggetto}
            ...
            className="w-full bg-slate-50 border-2 border-transparent focus:border-blue-600 rounded-2xl px-6 py-4 text-sm focus:outline-none transition-all font-bold text-slate-900 placeholder:text-slate-300" />
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">Numero Documento *</label>
            <input type="text" name="numero" required defaultValue={mockData?.numero}
              placeholder="Es. FPA-12/26"
              className="w-full bg-slate-50 border-2 border-transparent focus:border-blue-600 rounded-2xl px-6 py-4 text-sm focus:outline-none transition-all font-bold text-slate-900 placeholder:text-slate-300" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">Data Scadenza</label>
            <input type="date" name="dataScadenza" defaultValue={mockData?.dataScadenza}
              className="w-full bg-slate-50 border-2 border-transparent focus:border-blue-600 rounded-2xl px-6 py-4 text-sm focus:outline-none transition-all font-bold text-slate-900" />
          </div>
        </div>

        <div>
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">Data Emissione *</label>
      ...
      <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">Imponibile (€) *</label>
            <input type="number" name="importo" required step="0.01" value={importo} onChange={e => setImporto(e.target.value)}
              className="w-full bg-white border-2 border-transparent focus:border-blue-600 rounded-2xl px-6 py-4 text-lg focus:outline-none transition-all font-bold text-slate-900" />
          </div>
          <div>
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">IVA (%) *</label>
            <select name="iva" value={iva} onChange={e => setIva(e.target.value)} className="w-full bg-white border-2 border-transparent focus:border-blue-600 rounded-2xl px-6 py-4 text-sm focus:outline-none transition-all font-bold text-slate-900">
        ...
        <div className="pt-2 flex justify-between items-center bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Totale a pagare</span>
          <span className="text-2xl font-bold text-slate-900 tracking-tighter">€ {totale.toLocaleString('it-IT', { minimumFractionDigits: 2 })}</span>
        </div>
      </div>

      {/* Cantieri */}
      <div>
        <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wide block mb-2 ml-1">Associa ai Cantieri</label>
      ...
      <div className="pt-8">
        <button type="submit" disabled={isPending}
          className="w-full bg-slate-900 hover:bg-blue-600 disabled:opacity-50 text-white py-5 rounded-2xl font-bold text-[12px] uppercase tracking-wide shadow-xl transition-all transform active:scale-95 flex items-center justify-center gap-2">
