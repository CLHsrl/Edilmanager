  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white border border-gray-100 p-4 rounded-2xl shadow-sm">
          <div className="flex items-center gap-2 text-blue-600 mb-2">
            <Umbrella size={16} />
            <span className="text-[10px] font-bold uppercase">Saldo Ferie</span>
          </div>
          <p className="text-2xl font-bold text-gray-900">{saldoFerie} <span className="text-xs font-bold text-gray-400">giorni</span></p>
        ...
        <div className="bg-white border border-gray-100 p-4 rounded-2xl shadow-sm">
          <div className="flex items-center gap-2 text-purple-600 mb-2">
            <Timer size={16} />
            <span className="text-[10px] font-bold uppercase">Permessi Fruiti</span>
          </div>
        ...
        {isAddingAbsence ? (
          <form onSubmit={handleAbsenceSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div />
              <div>
                <label className="text-[9px] font-bold text-gray-400 uppercase block mb-1">Inizio</label>
              ...
              <div>
                <label className="text-[9px] font-bold text-gray-400 uppercase block mb-1">Fine</label>
            ...
            <div className="flex gap-2">
              <button type="submit" className="flex-1 bg-blue-600 text-white py-3 rounded-xl font-bold text-xs uppercase tracking-wide shadow-lg shadow-blue-100">Invia</button>
              <button type="button" onClick={() => setIsAddingAbsence(false)} className="px-4 bg-gray-200 text-gray-500 rounded-xl font-bold text-xs uppercase tracking-wide">Annulla</button>
            </div>
          </form>
        ) : (
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide pl-1">Attività Assegnate</h4>
        <div className="bg-white border border-gray-100 rounded-2xl divide-y divide-gray-50 overflow-hidden shadow-sm">
          {lavoratore.tasks?.map((task: any) => (
            <div key={task.id} className="p-4 flex justify-between items-center hover:bg-gray-50 transition-colors">
              ...
              <span className={`text-[9px] font-bold px-2 py-1 rounded-lg uppercase tracking-wider border ${
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide pl-1">Compliance & Sicurezza</h4>
        <div className="space-y-2">
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide pl-1">Cronologia Presenze</h4>
          ...
          {lavoratore.presenze?.slice(0, 5).map((p: any) => (
            <div key={p.id} className="p-4 flex justify-between items-center bg-white hover:bg-gray-50 transition-colors">
              ...
              <div className="text-right">
                ...
                {p.latIn && (
                  <p className="text-[8px] text-gray-300 mt-0.5">{p.latIn.toFixed(4)}, {p.longIn.toFixed(4)}</p>
                )}
              </div>
            </div>
          ))}
